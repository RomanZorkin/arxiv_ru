...
```
в каждом каталоге находиться зеркало оригинальной статьи, ее перевод на русский язык и метаданные. Источником является ar5iv.org (или точнее ar5iv.labs.arxiv.org) — это проект, который предоставляет статьи из arXiv.org в удобном формате responsive HTML5 (а не только в PDF). Иногда ar5iv.org не успевает преобразовать свежие статьи, тогда родительский HTML берется из arxiv.org
...
- Явные ошибки и не согласованность дорабатываются в ручную
- Полученный первеод - не является точным техническим переводом, часть недочетов сохрнаяется
- Безусловно, лучше читать вместе с оригиналом для получения полного контекста

## Возможность участия в проекте
Вы можете предлагать интересные статьи для перевода, свои переводы, создавать предложения по исправлению замеченных ошибок и конечно активно обсуждать.
